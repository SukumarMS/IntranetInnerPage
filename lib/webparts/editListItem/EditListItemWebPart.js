...
import { readItems, updateitems, checkUserinGroup, GetQueryStringParams, base64ToArrayBuffer } from '../../commonService';
var EditListItemWebPart = (function (_super) {
    ...
    EditListItemWebPart.prototype.UpdateItem = function (siteURL, strLocalStorage, strComponentId) {
        ...
        if (strLocalStorage == "Announcements") {
            ...
            if (files.files.length == 0) {
                ...
                if (isAllfield) {
                    $body.addClass("loading");
                    updateitems("Employee Corner", strComponentId, saveData_1, function (e) {
                        if (e.data) {
                            $body.removeClass("loading");
                            that.pageBack();
                        }
                ...
                if (isAllfield) {
                    ...
                    $body.addClass("loading");
                    pnp.sp.web.getFolderByServerRelativeUrl("Shared Documents").files.add(file_1.name, file_1, true)
                        .then(function (result) {
            ...
            if (files.files.length == 0) {
                ...
                if (isAllfield) {
                    $body.addClass("loading");
                    updateitems("Organizational Policies", strComponentId, saveData_2, function (e) {
                        if (e.data) {
                            $body.removeClass("loading");
                            that.pageBack();
                        }
            ...
            if (this.qustion_lenght != updatequs || this.answer_lenght != updateans) {
                pnp.sp.web.lists.getByTitle("PollsResults").items.filter("QuestionID eq '" + strComponentId + "'").get().then(function (items) {
                    for (var i = 0; i < items.length; i++) {
                        pnp.sp.web.lists.getByTitle("PollsResults").items.getById(items[i].ID).delete().then(function (result) {
                            var myobjPols = {
                                Question: $("#txtQuestion").val(),
                                Options: $("#txtOptions").val(),
                                IsVisibles: $('#anonymous').prop('checked')
            ...
            if (files.files.length == 0 && docfiles.files.length == 0) {
                ...
                if (isAllfield) {
                    $body.addClass("loading");
                    updateitems("Corporate Discounts", strComponentId, saveData_3, function (e) {
                        if (e.data) {
                            $body.removeClass("loading");
                            that.pageBack();
                        }
                ...
                pnp.sp.web.getFolderByServerRelativeUrl("PublishingImages").files.add(file.name, blob, true)
                    .then(function (result) {
                    pnp.sp.web.getFolderByServerRelativeUrl("Shared Documents").files.add(docfile_1.name, docfile_1, true)
    ...
    EditListItemWebPart.prototype.bindorgDept = function () {
        var objResults = readItems("Departments", ["ID", "Departments"], 1000, "Modified", "Display", 1);
